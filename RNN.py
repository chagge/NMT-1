...
class LangModel(Opt):
  ...
  def __init__(self, libs, dims, **kwargs):
    ...
    for lib in libs:
      ...
      if self.shareLibs:
        assert lib.L.get_value().shape[0] == dims[-1]
    ...
    if self.model.endswith('LSTM'):
      self.C_0 = [theano.shared(np.zeros_like(h_0_l.get_value()), name='C'+h_0_l.name[1:]) for h_0_l in h_0]
      self.outputs_info = self.h_0 + yhat_0 + self.C_0 + yhat_0
    else:
      self.outputs_info = self.h_0 + yhat_0
    self.params =\
        self.Wparams +\
        self.Uparams +\
        self.bparams +\
    ...
    for i, lib in enumerate(self.libs)
      self.lparams.append(lib.get_subtensor(self.idxs[:-1,i]))
